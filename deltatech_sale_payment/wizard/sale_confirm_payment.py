...
class SaleConfirmPayment(models.Model):
    ...
    def api_data(self,active_id):
        for rec in self:
            ...
            if tx and tx.state in ["pending", "authorized"]:
                ...
                erc.acquirer_id= tx.acquirer_id.id
                rec.amount = tx.amount
            if rec.amount <= 0:
                raise UserError(_("Then amount must be positive"))

            if rec.transaction_id and rec.transaction_id.amount == rec.amount:
            ...
            if transaction.state != "done":
                transaction = transaction.with_context(payment_date=rec.payment_date)
